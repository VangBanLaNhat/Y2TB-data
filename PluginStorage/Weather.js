...
function init(){
    return{
        ...
        "author": "HerokeyVN",
        "version": "0.0.1",
        "apiKey": "17bb6afb9c46bbee42211f1c3e975103"
    }
}
...
async function wth(data, api){
    ...
    /*if (global.data.weather[data.threadID][data.senderID]) api.sendMessage(lang.plssend[lc].replace("{0}", min), data.threadID, data.messageID)
        ...
        try{
            ...
            var link = `http://api.openweathermap.org/data/3.0/onecall?q=${encodeURIComponent(msg)}&lang=${global.config.bot_info.lang.split("_")[0]}&units=metric&appid=${init().apiKey}`;
          try{
            var fdt = (await axios(link)).data
          }catch(e){
            api.sendMessage(e.response.data.message, data.threadID, data.messageID);
...
async function rslc(data, api){
    if(data.type == "message" && data.attachments[0]) if(data.attachments[0].type == "location") if(global.data.weather[data.threadID][data.senderID]){
        ...
        var axios = require("axios");
        var link = `https://api.openweathermap.org/data/3.0/onecall?lat=${data.attachments[0].latitude}&lon=${data.attachments[0].longitude}&lang=${global.config.bot_info.lang.split("_")[0]}&units=metric&appid=${init().apiKey}`;
        try{
          var fdt = (await axios(link)).data
        }catch(e){
          api.sendMessage(e.response.data.message, data.threadID, data.messageID);
